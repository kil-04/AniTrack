...
val trustJson = rootProject.file("../../packages/shared/automation-trust.json").readText()
fun trustValue(name: String): String = Regex("\"$name\"\\s*:\\s*\"([^\"]+)\"")
    .find(trustJson)?.groupValues?.get(1)
    ?: throw GradleException("Missing $name in packages/shared/automation-trust.json")
