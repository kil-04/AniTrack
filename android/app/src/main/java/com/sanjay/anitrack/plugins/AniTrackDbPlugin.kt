...
class AniTrackDbPlugin : Plugin() {
    ...
    private fun buildContinueWatching(rdb: SQLiteDatabase, page: Int, pageSize: Int): JSONObject {
        ...
        // Count distinct anime (same as Electron — no completion filter, always show latest touched)
        val countC = rdb.rawQuery(
            "SELECT COUNT(DISTINCT anime_id) FROM playback WHERE duration_sec > 0",
            null)
        val total = if (countC.moveToFirst()) countC.getInt(0) else 0
        countC.close()

        val items = JSONArray()
        // CTE picks the single most-recently-touched episode per anime (matches Electron behavior)
        val c = rdb.rawQuery("""
            WITH latest AS (
            ...
            FROM playback p
            JOIN latest l ON l.anime_id = p.anime_id AND l.max_updated = p.updated_at
